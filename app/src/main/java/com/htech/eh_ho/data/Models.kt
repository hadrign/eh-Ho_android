...
data class  Topic(
    ...
    val MINUTE_MILLIS = 1000L * 60 //1000L: Operación en Long
    ...
    /**
     * Fecha del topic'01/01/2020 10:00:00'
     * @param Date Fecha del consulta '01/01/2020 10:10:00'
...
data class Post(
    ...
    companion object {
        ...
        fun parsePost(jsonObject: JSONObject): Post {
            ...
            val dateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ", Locale.getDefault())
            val dateFormatted = dateFormat.parse(date) ?: Date()
            val contentRaw = jsonObject.getString("cooked")
            val contentProcessed =
                contentRaw.replace("<p>", "").replace("</p>", "")
            return  Post(
                id = jsonObject.getInt("id").toString(),
